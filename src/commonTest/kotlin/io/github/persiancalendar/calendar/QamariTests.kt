...
class QamariTests {
    ...
    @Test
    fun `Conforms with Qamari tests`() = checkFile("qamari", TestResources.qamariConsolidated)

    @Test
    fun `Conforms with predictions`() = checkFile("predictions", TestResources.qamariPredictions)

    fun checkFile(fileName: String, content: String) {
        content
            .split("\n")
            .map { it.split("#")[0] }
